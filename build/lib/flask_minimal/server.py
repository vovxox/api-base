...
from flask_minimal.api import GetIssueEndpoint
...
class CustomFlaskApp(object):
    def __init__(self):
        ...
        custom_errors = {
            "JsonInvalidError": {"status": 500, "message": "JSON format not valid"},
            "JsonRequiredError": {"status": 400, "message": "JSON input required"},
        }
        self.api = swagger.docs(
            Api(self.app, errors=custom_errors), apiVersion=API_VERSION_NUMBER
        )

        self.api.add_resource(DummyEndpoint, "/dummy", endpoint="dummy")
        self.api.add_resource(HelloEndpoint, "/hello", endpoint="hello")
        self.api.add_resource(GetIssueEndpoint, "/issue", endpoint="issue")
